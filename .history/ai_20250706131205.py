...
def evaluate_model(model, n_samples=5):
    ...
    while len(X_test) < n_samples:
        # Oversample a bit to increase chance of getting valid roots
        X_batch, y_batch = generate_data(n_samples * 2)
    ...
    X_test = torch.stack(X_test[:n_samples])
    y_real = torch.stack(y_real[:n_samples])
