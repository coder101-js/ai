...
def generate_data(n, seed=None):
    ...
    b = rng.uniform(-15, 20, n)
    c = rng.uniform(-20, 200, n)
    ...
    for ai, bi, ci in zip(a, b, c):
        ...
        data.append([ai / 10, bi / 20, ci / 200])
...
def train(max_minutes=30):
    ...
    # New, bigger dataset
    total_samples = 7_500_000  # 👈 increased dataset
    batch_size = 2048
    max_epochs = 2000
    patience = 10
    bad_epochs = 0
...
def evaluate_model(model, n_samples=5):
    ...
    while len(X_test) < n_samples:
        X_batch, y_batch = generate_data(n_samples * 3)
    ...
    if len(X_test) < n_samples:
        print(f"⚠️ Only got {len(X_test)} valid test samples.")
        return
